...
studies introduced the idea that networks can vary from one realization to
...
name, and their identifiers in a variety of taxonomic services (EOL,
BGIF, ITIS, NCBI). Associating the identifiers of each taxa is important to
leverage the power of the new generation of open data tools, such as `taxize`
[@cha13a]. The data specification currently has fields for `ncbi`, `gbif`,
`itis`, `eol` and `bold` identifiers. We also provide the taxonomic status,
*i.e.* whether a taxa is a true taxonomic entity, a "trophic species",
or a morphospecies.
