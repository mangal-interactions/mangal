...
the structure of ecological networks [@pois12c;@pois14]. To date, meta-analyses of
...
systems [@schl11;@dals13;@pois13;@cham14;@olit14]. As such, the severe shortage of data in
...
the structure of ecological networks, either by relying on latent variables
...
of different methods is also facilitated if there is a homogeneous way of
...
(iii) describe `rmangal`, an `R` package and companion database relying on this
data specification. The `rmangal` package allows to easily retrieve, but also
deposit, ecological interaction network data from a common database. We provide some
use cases showing how this new approach makes complex analyses simpler and
...
which can be  either `individual` or `population`; this allows for the representation of
both individual-level networks (*i.e.* there are as many `item`s of a
given taxa as there were individuals sampled), and population-level
...
species and the number of interactions, which @mart92 argued should be linear
(in food webs).
...
In this contribution, we have introduced `mangal`, a data format for the
exchange of ecological networks and associated meta-data. We further deployed
...
the moment, users are welcome and encouraged to propose such changes on the project
issue page: <https://github.com/mangal-wg/mangal-schemes/issues>. Though we have not described it here, a 
