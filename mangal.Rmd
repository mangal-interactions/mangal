...
`http://mangal.io/doc/spec/`. The data specification is available
either at the API root (*e.g.* `http://mangal.io/api/v1/?format=json`),
...
data through an API (`http://mangal.io/api/v1/`). It is possible for users
...
`http://mangal.io/api/v1/dataset/1`. The `rmangal` package can be installed this way:
...
<http:/mangal.io/dashboard/login>. Your API key be generated automatically
...
install_github(repo='betalink', username='tpoisot')
library(betalink)
